...
# THEANO_FLAGS=mode=FAST_RUN,device=gpu,floatX=float32 python lstm_encoder.py
...
def train_test_split(data,target, split=0.15):
    ...
    train_data = data.take(perms[0:train_n],axis=0)

    test_data = data.take(perms[train_n:],axis=0)
...
(X_train, X_test) = train_test_split(data_x, data_y, split=0.15)
# model
...
nm_hidden = 128
model = Sequential()
model.add(LSTM(nm_hidden,input_dim=nm_input))
model.add(RepeatVector(n_timesteps))
model.add(LSTM(nm_output, return_sequences=True))
model.add(TimeDistributedDense(nm_output))
checkpointer = ModelCheckpoint(filepath="bestmodel_swissprot_lstm.hdf5", verbose=1, save_best_only=True)
...
model.load_weights('bestmodel_swissprot_lstm.hdf5')
